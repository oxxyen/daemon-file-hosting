...
// --- Вспомогательные функции для надежной отправки/получения данных ---

/**
 * @brief Надежно отправляет все байты из буфера.
...
/**
 * @brief Надежно получает все байты в буфер.
 * @param sockfd Файловый дескриптор сокета.
 * @param buffer Указатель на буфер для получения данных.
 * @param len Количество байт для получения.
 * @return 0 в случае успеха, -1 в случае ошибки.
 */
int recv_all(int sockfd, void *buffer, size_t len) {
    ...
    while (total_received < len) {
        ...
        if (bytes_received == 0) {
            ...
            return -1; // Соединение закрыто до получения всех данных
        }
...
// --- Функции для операций с файлами ---
